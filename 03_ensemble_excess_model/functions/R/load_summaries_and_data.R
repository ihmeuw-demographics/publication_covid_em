...
load_summaries_and_data <- function(run_ids,
  ...
  dt <- rbindlist(lapply(run_ids, function(run_id) {
    ...
    if (length(loc_list) > 1) {
      temp <- assertable::import_files(
        filenames = c(paste0(c(2010:2021), ".csv"), paste0(loc_list, ".csv")),
        folder = folder,
        warn_only = T
      )
    } else {
  ...
  data <- rbindlist(lapply(run_ids, function(run_id) {

    # load in data for one run
    data_files <- paste0(unique(dt$ihme_loc_id), ".csv")
    temp <- assertable::import_files(
      filenames = data_files,
      folder = fs::path(base_dir, run_id, "/inputs/data_all_cause_not_offset/")
    )

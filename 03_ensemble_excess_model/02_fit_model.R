...
#   4. Save draws
# Inputs:
#   * Processed data: {main_dir}/inputs/data_all_cause/{ihme_loc_id}.csv
# Outputs:
#   * {main_dir}/outputs/draws/{model_type}/{process_stage}/{groupings}/
#         {location_id==loc_id}/{year_start}/draws.parquet
...
# still do 1000 draws here if not doing draw-level for regmod
...
# assign `process_stage` to be used for partitioning draw files:
#   final: final estimates that are post-scaling/aggregation or don't need it.
draws[, process_stage := "final"]

# assign `groupings` to be used for partitioning draw files:
#   estimated: directly estimated sexes and ages.

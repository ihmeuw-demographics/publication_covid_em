...
source(paste0(data_code_dir, "FILEPATH"))
source(paste0(data_code_dir, "FILEPATH"))
...
india_raw_file_dir <- "FILEPATH"
if(GBD) {
  ind_custom_orig <- fread(paste0("FILEPATH"))
} else {
  ind_custom_orig <- fread(paste0("FILEPATH"))
}
...
if (GBD) {
  ...
  sero <- sero[!location_id %in% c(90, 214)] #just in case
  sero <- rbind(sero, replace_sero)

  daily_covid <- fread("FILEPATH")
  start_dt <- fread( "FILEPATH")
...
ext_dir <- fs::path(main_dir, "FILEPATH")
...
sero[, draw := gsub("infections_draw_", "", draw)]
sero[, draw := as.numeric(draw) + 1]
...
if (GBD) {
  sero_sub_locs_1 <- sort(locs[(ihme_loc_id %like% "IND|KEN" & level == 4) | (ihme_loc_id %like% "GBR" & level == 5)]$ihme_loc_id)
} else {
...
for (dataset_type in dataset_types) {
  ...
  # fix iran since it is only one point
  ...
  # get custom india times
  ind <- fread(india_raw_file_dir)
  ind <- ind[!is.na(excess_deaths)] # only need rows with deaths excess
  ...
  keep_dates_sep <- unique(ind[!is.na(excess_deaths), c("location_id", "date_start", "date_end")])
  ...
  for (loc_time in 1:nrow(keep_dates_sep)) {
    ...
    temp[, ':=' (time_start = month(date),
                 year_start = year(date),
                 time_unit = "month",
                 ihme_loc_id = paste0("IND_",location_id),
                 keep = 1)]
    temp <- unique(temp[, c("location_id","ihme_loc_id","time_start","time_unit","year_start","keep")])
    keep_dates <- rbind(keep_dates, temp)

  }

  # replace india values
  keep_times <- rbind(keep_times[!location_id %in% unique(keep_dates$location_id)], keep_dates)
  ...
  if (dataset_type == "all") {
    ...
    death_rate_excess <- death_rate_excess[(time_unit == "week" & ((time_start >= start_week & year_start == start_year) | year_start > start_year) ) | (time_unit == "month" & ((time_start >= start_month & year_start == start_year) | year_start > start_year))]
    # cut out after time cutoff
    death_rate_excess <- death_rate_excess[(year_start < year_cutoff) | (time_unit == "week" & (year_start == year_cutoff & time_start <= week_cutoff)) | (time_unit == "month" & (year_start == year_cutoff & time_start <= month_cutoff))]

    death_rate_excess <- death_rate_excess[, -c("start_year", "start_month", "start_week")]

  } else if (dataset_type == "only2020") {

    death_rate_excess <- death_rate_excess[year_start == 2020 & ((!location_id %in% c(6, chn_loc_ids) & ((time_unit == "week" & time_start >= 9) | (time_unit == "month" & time_start >= 3))) | (location_id %in% c(6, chn_loc_ids) & ((time_unit == "week" & time_start >= 1) | (time_unit == "month" & time_start >= 1))))]
  ...
  death_rate_excess <- death_rate_excess[outlier == FALSE,
  ...
  if (dataset_type == "all") {
    ...
    sero <- sero[((!location_id %in% c(6, chn_loc_ids)) & (date >= as.Date("2020-03-01")) | (location_id %in% c(6, chn_loc_ids))) & date <= as.Date("2020-12-31")]
  ...
  if (dataset_type == "custom") {
    ...
    sero <- merge(
      sero,
      pop_sub[time_unit == "day", c("location_id","person_years","date","draw")],
      by = c("location_id","date","draw"),
  ...
  draw_level_covs <- merge(
    draw_level_covs,
    dt_em[, c("location_id","person_years","death_rate_covid","deaths_covid","covid_person_years","stars",
  ...
  if(dataset_type == "only2021") {
    ...
    # get draws
    dt_prop <- merge(
      draw_level_covs[, c("location_id", "draw")],
      dt_prop,
      by = "location_id",
      all.y = TRUE
    )
    dt_prop[location_id %in% nor_loc_ids, ':=' (nat_cumulative_infections = rep(draw_level_covs[location_id == 90, cumulative_infections], 11),
                                                nat_cumulative_infections_lagged = rep(draw_level_covs[location_id == 90, cumulative_infections_lagged], 11),
                                                nat_death_rate_covid = rep(draw_level_covs[location_id == 90, death_rate_covid], 11))]
    dt_prop[location_id %in% nga_loc_ids, ':=' (nat_cumulative_infections = rep(draw_level_covs[location_id == 214, cumulative_infections], 37),
                                                nat_cumulative_infections_lagged = rep(draw_level_covs[location_id == 214, cumulative_infections_lagged], 37),
                                                nat_death_rate_covid = rep(draw_level_covs[location_id == 214, death_rate_covid], 37))]
    ...
    draw_level_covs <- merge(
      draw_level_covs,
      dt_prop[, c("location_id","draw","calc_cumulative_infections","calc_cumulative_infections_lagged","calc_death_rate_covid")],
      by = c("location_id", "draw"),
      all.x = TRUE
    )
    draw_level_covs[location_id %in% c(nor_loc_ids, nga_loc_ids) & is.na(cumulative_infections), cumulative_infections := calc_cumulative_infections]
    draw_level_covs[location_id %in% c(nor_loc_ids, nga_loc_ids) & is.na(cumulative_infections_lagged), cumulative_infections_lagged := calc_cumulative_infections_lagged]
    draw_level_covs[location_id %in% c(nor_loc_ids, nga_loc_ids) & is.na(death_rate_covid), death_rate_covid := calc_death_rate_covid]

    draw_level_covs <- draw_level_covs[, -c("calc_cumulative_infections","calc_cumulative_infections_lagged","calc_death_rate_covid")]
  }
  ...
  if (dataset_type == "all") {
    ...
    pop_agg <- pop_agg[time_unit == "week" & (!(year_start == year_cutoff & time_start > week_cutoff) & !(year_start > year_cutoff) & !(year_start < 2020) & !(year_start == 2020 & time_start <= 9 & !location_id %in% c(6, chn_loc_ids)))]

  } else if (dataset_type == "only2020") {

    pop_agg <- pop_agg[year_start == 2020 & ((!location_id %in% c(6, chn_loc_ids) & (time_unit == "week" & time_start >= 9)) | (location_id %in% c(6, chn_loc_ids) & (time_unit == "week" & time_start >= 1)))]

  } else if (dataset_type == "only2021") {

    pop_agg <- pop_agg[year_start == 2021]
    ...
    pop_agg <- pop_agg[(keep == 1)]

  }

  # aggregate
  pop_agg <- pop_agg[, .(person_years = sum(person_years)), by = "location_id"]

  dt_chn <- draw_level_covs[location_id %in% chn_loc_ids]
  dt_chn <- dt_chn[!location_id == 503]

  # add person years
  dt_chn[, person_years := NULL]
  dt_chn <- merge(
    dt_chn,
    pop_agg[, c("location_id","person_years")],
    by = "location_id",
    all.x = TRUE
  )

  dt_chn <- dt_chn[, .(idr_reference = weighted.mean(idr_reference, person_years),
  ...
  draw_level_covs[location_id == 492,
                  ':=' (idr_reference = (dt_chn$idr_reference),
                        idr_lagged = (dt_chn$idr_lagged),
                        mobility_reference = (dt_chn$mobility_reference),
                        mobility_lagged = (dt_chn$mobility_lagged),
                        cumulative_infections = (dt_chn$cumulative_infections),
                        cumulative_infections_lagged = (dt_chn$cumulative_infections_lagged),
                        ci_person_years = (dt_chn$ci_person_years),
                        crude_death_rate = (dt_chn$crude_death_rate),
                        crude_death_rate_sd_1990 = (dt_chn$crude_death_rate_sd_1990),
                        crude_death_rate_sd_2000 = (dt_chn$crude_death_rate_sd_2000),
                        avg_abs_latitude = (dt_chn$avg_abs_latitude),
                        cirrhosis_death_rate = (dt_chn$cirrhosis_death_rate),
                        ckd_death_rate = (dt_chn$ckd_death_rate),
                        cong_downs_death_rate = (dt_chn$cong_downs_death_rate),
                        cvd_death_rate = (dt_chn$cvd_death_rate),
                        cvd_pah_death_rate = (dt_chn$cvd_pah_death_rate),
                        cvd_stroke_cerhem_death_rate = (dt_chn$cvd_stroke_cerhem_death_rate),
                        diabetes_death_rate = (dt_chn$diabetes_death_rate),
                        endo_death_rate = (dt_chn$endo_death_rate),
                        gbd_obesity = (dt_chn$gbd_obesity),
                        HAQI = (dt_chn$HAQI),
                        hemog_sickle_death_rate = (dt_chn$hemog_sickle_death_rate),
                        hemog_thalass_death_rate = (dt_chn$hemog_thalass_death_rate),
                        hiv_death_rate = (dt_chn$hiv_death_rate),
                        ncd_death_rate = (dt_chn$ncd_death_rate),
                        neo_death_rate = (dt_chn$neo_death_rate),
                        neuro_death_rate = (dt_chn$neuro_death_rate),
                        resp_asthma_death_rate = (dt_chn$resp_asthma_death_rate),
                        resp_copd_death_rate = (dt_chn$resp_copd_death_rate),
                        smoking_prevalence = (dt_chn$smoking_prevalence),
                        subs_death_rate = (dt_chn$subs_death_rate),
                        universal_health_coverage = (dt_chn$universal_health_coverage))]

  draw_level_covs <- draw_level_covs[!location_id %in% c(6, 44533, setdiff(chn_loc_ids, c(503,492)))]
  ...
  if (dataset_type == "custom") {
    ...
    keep_times <- keep_times[!location_id %in% c(35498,35499)]

    # subset to only locations needed
    draw_level_covs <- draw_level_covs[location_id %in% unique(keep_times$location_id)]

  }

  # Run checks -----------------------------------------------------------------

  # drop china subnat and ken admin 4 subnats
  drop_locs <- c(44533, gbd_loc_map[ihme_loc_id %like% "KEN_" & level == 4]$location_id)
  draw_level_covs <- draw_level_covs[!location_id %in% drop_locs]

  # drop global and region
  ...
  if (GBD) {
    if (dataset_type == "all") {
      ...
      readr::write_csv(draw_level_covs, paste0(out_dir, "FILEPATH"))

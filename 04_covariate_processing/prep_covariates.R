...
source("FILEPATH/pop_to_person_years.R")

# Set parameters ----------------------------------------------------------

out_dir <- fs::path("FILEPATH")
...
## NOTE:
#  Get age and sex specific values from:
#  FILEPATH
#  and run `exposure_prevalence.R`

dt_hypertension_raw <- fread(fs::path(out_dir, "raw_hypertension_prevalence.csv"))
...
# use 2019 population for this data to be consistent
...
# smoking
dt_smoking <- fread(fs::path(out_dir, "gbd2019_num_smokers_both_sex_all_ages.csv"))
dt_smoking <- merge(
  dt_smoking,
...
dt_smoking[, mean := NULL] #dropping to prevent later confusion
...
# GBD Obesity
dt_obesity <- fread(fs::path(out_dir, "/all_age_both_sex_prev_obesity_gbd_2019.csv"))
...
covid_team_pop <- setDT(cbind(covid_team_pop, year_start = rep(2018:2022, each = nrow(covid_team_pop))))
...
readr::write_csv(array_locs, paste0("FILEPATH"))
readr::write_csv(all_population, paste0("FILEPATH"))

# submit qsub
image_dir <- "FILEPATH"
image_gbd_year_release <- "2021.1"

source("FILEPATH")

qsub(
  jobname = "interpolate_pop",
  shell = glue::glue("FILEPATH"),
  code = "FILEPATH",
  fthread = 1, fmem = 2, h_rt = "00:10:00", archive = F,
  pass_shell = list(y = image_gbd_year_release),
  pass_argparse = list(task_map_path = paste0("FILEPATH"),
                       date = date),
  num_tasks = njobs,
  queue = "all",
...
pop_files <- list.files(paste0("FILEPATH"), pattern = ".csv", full.names = T, recursive = T)
pop_all <- lapply(pop_files, fread) %>% rbindlist(use.names = T, fill = T)

readr::write_csv(pop_all, paste0("FILEPATH"))

# save final version
readr::write_csv(pop_all, paste0(out_dir, "FILEPATH"))
readr::write_csv(pop_all, fs::path(out_dir, "/estimated_person_years.csv"))
...
readr::write_csv(dt_mean_pop_age, fs::path(out_dir, "mean_pop_age_GBD2019.csv"))
readr::write_csv(dt_crude_death_rate_2019, fs::path(out_dir, "crude_death_rate_GBD2019.csv"))
readr::write_csv(dt_crude_death_rate_sd_1990, fs::path(out_dir, "crude_death_rate_sd_1990_GBD2019.csv"))
readr::write_csv(dt_crude_death_rate_sd_2000, fs::path(out_dir, "crude_death_rate_sd_2000_GBD2019.csv"))
...
for (age in c(60, 70, 75, 80, 85)) {
  ...
  readr::write_csv(
    ...
    fs::path(out_dir, paste0("proportion_pop_", age, "plus_GBD2019.csv"))
  )
...
purrr::iwalk(
  dt_gbd_causes_list,
  ~readr::write_csv(.x, fs::path(out_dir_gbd, paste0(.y, "_death_rate_GBD2019.csv")))
)

readr::write_csv(dt_hypertension_total_2019, fs::path(out_dir_gbd, "hypertension_prevalence_GBD2019.csv"))
readr::write_csv(dt_inpatient_admis, fs::path(out_dir_gbd, "gbd_inpatient_admis_GBD2020.csv"))
readr::write_csv(dt_diabetes, fs::path(out_dir_gbd, "gbd_diabetes_GBD2019.csv"))
readr::write_csv(dt_smoking, fs::path(out_dir_gbd, "smoking_prevalence_GBD2019.csv"))
readr::write_csv(dt_uhc, fs::path(out_dir_gbd, "universal_health_coverage_GBD2019.csv"))
readr::write_csv(dt_lat, fs::path(out_dir_gbd, "avg_abs_latitude_GBD2019.csv"))
readr::write_csv(dt_haqi, fs::path(out_dir_gbd, "HAQI_GBD2019.csv"))
readr::write_csv(dt_obesity, fs::path(out_dir_gbd, "gbd_obesity_GBD2019.csv"))

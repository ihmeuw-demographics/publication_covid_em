...
if (process_allcause) {
  allcause <- fread(paste0(main_dir, "FILEPATH"))
  allcause <- allcause[ihme_loc_id == ihme_loc]
  allcause[, location_id := loc_id]
  wmd <- fread(paste0(main_dir, "FILEPATH"))
  wmd <- wmd[location_id == loc_id]
  completeness <- fread(fs::path(main_dir, "FILEPATH"))
...
if(process_em){
  ext_em <- fread(paste0(main_dir, "FILEPATH"))
...
# person years
person_years <- fread(paste0(main_dir, "FILEPATH"))
person_years <- person_years[location_id == loc_id]

# covid
covid <- fread(paste0(main_dir, "FILEPATH"))
covid <- covid[location_id == loc_id]
covid <- covid[year_start %in% covid_years]

# idr cov
idr <- fread(paste0(main_dir, "FILEPATH"))
if (loc_id == 99999) {
...
# seir covs
# testing cov - proportin of pop tested?
testing <- fread(paste0(main_dir,"FILEPATH"))
if (loc_id == 99999) {
...
# mobility cov - percent change in mobility from previous year
mobility <- fread(paste0(main_dir,"FILEPATH"))
if (loc_id == 99999) {
...
gbd_inpatient <- fread(paste0(main_dir,"FILEPATH"))
if (loc_id == 99999) {
...
# vr star rating
stars <- fread(paste0(main_dir,"FILEPATH"))
if (loc_id == 99999) {
...
if (process_allcause) {
  ...
  # PRT HMD missing 95+ age group. For now just combine w/ 90-95.
  if (ihme_loc == "PRT") {
    # allcause <- allcause[week_start <= 52]
  ...
  # BRA_4771 has deaths > pop for 95+, so collapse 90-94 and 95+
  if (ihme_loc == "BRA_4771") {
    allcause[age_start >= 90, `:=` (age_start = 90, age_end = 125)]
  }

  # sex and age specific eurostat data breaking the aggregation
  if(allcause$source[1] == "EUROSTAT"){
    allcause <- allcause[!sex=="all" & (age_start==0 & age_end==125)]
  }

  # sex and age specific THA data breaking the aggregation
  if(ihme_loc == "THA"){
    allcause <- allcause[sex=="all" & (age_start==0 & age_end==125)]
  }
...
if (process_allcause) {
  ...
  # Locations missing terminal age groups in later weeks are not
  # aggregating them - going to drop
  ...
  # VR adjustment for countries outside of high-income
  # combine with processed VR data
    ...
    allcause[age_end > 5, deaths := deaths / adult]
    allcause[, c("adult", "child") := NULL]
  ...
  if (("week" %in% unique(allcause$time_unit)) &
      !("month" %in% unique(allcause$time_unit)) | ihme_loc == "TWN") {
  ...
  # Identify if WMD has more recent data and if so, add
  if (nrow(wmd) > 0) {
    ...
    if (loc_id == 71) { # Australia b/c they report "Doctor Certified" deaths (~10-15% below total)
    ...
    if (nrow(keep) > 0) {
      ...
      # save
      readr::write_csv(
        keep, fs::path(main_dir, "FILEPATH"))
      )
...
if (!process_allcause & nrow(person_years) > 0) {
...
# This section finds person years for external em data that may be only a single time unit
...
if (nrow(covid) == 0) {
  ...
  if(nrow(person_years) > 0) {
    ...
    # Adds NA rows for missing age/sex group combinations in the covid data to
    # align with allcause and person_years data
  ...
  if (process_em) {
    # external locations other than ZAF need to be cumulative to get covid deaths
    # over 1 to end of time start period
...
# using raw data #
if (ihme_loc %like% "ETH_"){
  # use national covid_incidence to weight ETH subnationals - daily cases will be for national level
...
if (process_em) {
  ...
  data_ext <- data_ext[!(year_start > 2021)]
  assertthat::assert_that(
    all(data_ext[!is.na(deaths_excess)]$deaths_excess < data_ext[!is.na(deaths_excess)]$person_years)
  )
...
if(process_em){
  readr::write_csv(
    data_ext, fs::path(main_dir, "FILEPATH")
  )
}

if(ihme_loc != "Mumbai"){
  readr::write_csv(
    data, fs::path(main_dir, "FILEPATH")
  )
}
